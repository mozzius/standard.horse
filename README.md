...
- **[`@atproto/lex`](https://www.npmjs.com/package/@atproto/lex)** — lexicon
  codegen *and* the runtime `Client` for XRPC / records / blobs
...
> **You need an existing publication.** This first draft only *edits* existing
...
- **Development** uses an atproto *loopback* client. The OAuth server serves
...
`content` is an *open* union, so the markpub member is hand-typed in
...
- Writes Markdown **inline** (`text.markdown`). It *reads* blob-backed markdown
